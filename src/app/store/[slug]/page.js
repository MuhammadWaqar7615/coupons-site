...
export async function generateMetadata({ params }) {
  ...
  const res = await fetch(`${backendUrl}/api/stores/${encodeURIComponent(decodedSlug)}`, { next: { revalidate: 60 } });
  ...
  return {
    ...
    description: store.seoDescription || `Scopri le migliori offerte e codici sconto per ${store.name}.`,
  };
}
...
export default async function StorePage({ params }) {
  ...
  // The backend already serializes coupons and sets _id etc.
